...
DEFAULT_IF=$(route -n get default | grep interface | awk '{print $2}')
log "External interface: $DEFAULT_IF"
...
echo "nat on $DEFAULT_IF from 10.0.10.1/24 to any -> ($DEFAULT_IF)" | \
...
log "IPv4 NAT: $DEFAULT_IF (token: ${IPV4_TOKEN})"
...
#    reference when Wireguard is shut down.
echo "nat on $DEFAULT_IF from fd77:77:77:77::1/112 to any -> ($DEFAULT_IF)" | \
...
log "IPv6 NAT: $DEFAULT_IF (token: ${IPV6_TOKEN})"
